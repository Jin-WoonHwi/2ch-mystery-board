...
const { searchWebOpinions } = require('./webResearch');
...
async function generateArticle(overrideDate) {
  ...
  const webInsights = await searchWebOpinions(topic.title, topic.category, 4);
  ...
  if (webInsights.length > 0) {
    opBody += '\n\n■ネット上の意見・考察（要約）\n';
    webInsights.forEach((insight, idx) => {
      opBody += `・${idx + 1}. ${insight.snippet}（出典: ${insight.source}）\n`;
    });
  }
