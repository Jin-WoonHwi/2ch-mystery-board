...
const { fetchOnlineOpinions } = require('./webOpinions');
...
async function generateArticle(overrideDate) {
  ...
  // ネット上の意見・考察を取得（失敗時は空配列）
  const onlineOpinions = await fetchOnlineOpinions(topic, 5);
  ...
  if (onlineOpinions.length > 0) {
    opBody += '\n■ネット上で見かけた意見・考察\n';
    onlineOpinions.forEach((op) => {
      opBody += `・${op}\n`;
    });
  }
  ...
  const usedOnlineOpinions = new Set();
  ...
  for (let i = 2; i <= totalPosts; i++) {
    ...
    if (!generated && roll < 0.72 && onlineOpinions.length > 0) {
      // ネット上の意見・考察を引用したレス
      const onlineOpinion = pickUnused(onlineOpinions, usedOnlineOpinions, rng);
      if (onlineOpinion) {
        const intros = [
          '検索して出てきた意見だと、',
          'ネットの考察を拾うと、',
        ...
        content = `${pick(intros, rng)}${onlineOpinion}`;
        generated = true;
      }
    }
