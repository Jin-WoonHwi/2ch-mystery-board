...
const { fetchOnlineInsights } = require('./webOpinions');
...
async function generateArticle(overrideDate) {
  ...
  const onlineInsights = await fetchOnlineInsights(topic.title);
  ...
  if (onlineInsights.length > 0) {
    opBody += '\n■ネット上の意見・考察（公開情報の要約）\n';
    onlineInsights.slice(0, 3).forEach((insight) => {
      opBody += `・${insight}\n`;
    });
  }
  opBody += '\nお前らの考えを聞かせてくれ';
  ...
  const usedOnlineInsights = new Set();
  ...
  for (let i = 2; i <= totalPosts; i++) {
    ...
    if (!generated && roll < 0.80 && onlineInsights.length > 0) {
      const insight = pickUnused(onlineInsights, usedOnlineInsights, rng);
      if (insight) {
        const intros = [
          '海外掲示板だと、',
          'ネット上の考察で多いのは、',
        ...
        content = pick(intros, rng) + insight;
        generated = true;
      }
    }

    if (!generated && roll < 0.92) {
      // 別のレスへの返信
      if (i > 3) {
